...
COMMENT ON COLUMN chat_history.chat_id IS 'UUID to group related messages in a conversation';
...
CREATE OR REPLACE FUNCTION generate_chat_id()
RETURNS UUID AS $$
BEGIN
...
-- Create trigger to auto-generate chat_id if not provided
CREATE OR REPLACE FUNCTION set_chat_id()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.chat_id IS NULL THEN
        NEW.chat_id := generate_chat_id();
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_set_chat_id
