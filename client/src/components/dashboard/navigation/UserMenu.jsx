...
const UserMenu = ({ isExpanded = true }) => {
  ...
  const currentLanguage = settings?.language?.preferred || i18n.language;
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className={`absolute ${
        ...
        } w-76 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-600 z-50 overflow-hidden`}>
          
          {/* User info section - always show in dropdown */}
          <div className="p-4 border-b border-gray-100 dark:border-gray-700">
            <div className="flex items-center">
              {user?.profile_picture ? (
                <div className="w-12 h-15 rounded-full overflow-hidden mr-3">
                  <Image 
                    src={user.profile_picture}
                    alt={user.name || t('common.user', 'User')}
                    width={40}
          ...
          {showingAppearanceMenu && (
            <div>
              ...
              {/* Theme options */}
              {[
                { key: 'system', label: t('settings.theme.system', 'System'), icon: 'system' },
                { key: 'light', label: t('settings.theme.light', 'Light Mode'), icon: 'light' },
                { key: 'dark', label: t('settings.theme.dark', 'Dark Mode'), icon: 'dark' }
              ].map((themeOption) => (
                <button 
                  key={themeOption.key}
